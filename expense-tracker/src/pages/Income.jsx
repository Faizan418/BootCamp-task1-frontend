...
export default function Income() {
  ...
  // --- FIXED CHART & CATEGORY LOGIC ---
  const chartData = useMemo(() => {
    const categoryTotals = incomes.reduce((acc, curr) => {
      // FIX: Pehle category check karo, agar wo nahi hai to source check karo
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 border-b border-white/5 pb-8">
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all flex items-center gap-2 font-bold text-sm"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white/[0.03] border border-white/10 p-8 rounded-[2.5rem] flex flex-col justify-center items-center text-center">
            ...
            <h3 className="text-5xl font-black text-white">{chartData.length}</h3>
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-center bg-white/[0.02] border border-white/5 p-4 rounded-3xl backdrop-blur-md">
          <div className="relative flex-1 group w-full">
            ...
            <input
              ...
              className="w-full bg-transparent border border-white/10 rounded-2xl pl-12 pr-4 py-3 outline-none focus:ring-2 ring-emerald-500/20 transition-all text-sm font-medium"
            />
          </div>
          <div className="flex gap-3 w-full lg:w-auto">
            <select
              ...
              className="flex-1 lg:w-48 bg-slate-900 border border-white/10 rounded-2xl px-4 py-3 text-sm font-bold cursor-pointer outline-none appearance-none"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pb-20">
          ...
          <div className="lg:col-span-4">
            <div className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] p-8 sticky top-10">
               <div className="flex justify-between items-center mb-10">
                  <h4 className="font-black text-white flex items-center gap-2 text-sm uppercase tracking-widest">Allocation</h4>
                  {selectedChartCat && (
               ...
               <div className="h-64 relative">
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none w-full">
                  <p className="text-[10px] font-black text-slate-500 uppercase">Focus</p>
                  <p className="text-lg font-black text-white truncate px-4">{selectedChartCat || "Total"}</p>
                </div>
               </div>
              ...
              <AnimatePresence mode="popLayout">
                {filteredIncomes.map((t, index) => (
                  <motion.div
                    ...
                    className="group bg-white/[0.03] border border-white/5 hover:border-emerald-500/40 p-6 rounded-[2rem] flex flex-col justify-between transition-all"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-slate-900 flex items-center justify-center text-2xl shadow-xl border border-white/5">
                          {t.icon || "💰"}
                        </div>
                        <div>
                          <h5 className="font-black text-white text-sm group-hover:text-emerald-400 transition-colors">
                            {t.source || t.category}
                          </h5>
